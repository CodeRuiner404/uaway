...
add_action( 'after_setup_theme', 'karna_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function karna_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'karna_construct_sidebars' );
}

if ( ! function_exists( 'karna_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'karna_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function karna_woocommerce_start() { ?>
...
if ( ! function_exists( 'karna_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'karna_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function karna_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'karna_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'karna_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function karna_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'karna_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'karna_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'karna_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function karna_bbpress_css() {
...
if ( ! function_exists( 'karna_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'karna_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function karna_buddypress_css() {
...
if ( ! function_exists( 'karna_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'karna_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function karna_beaver_builder_css() {
