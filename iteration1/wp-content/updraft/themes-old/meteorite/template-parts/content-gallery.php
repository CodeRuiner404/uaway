<?php
/**
 * Template for Gallery post format on blog page, archives, search and single.
 *
 * @package Meteorite
 */
...
// Grab gallery from the post content
$gallery = hybrid_media_grabber_raw( array( 'type' => 'gallery', 'split_media' => true, 'before' => '<div class="entry-media">', 'after' => '</div>' ) );
?>
...
<article id="post-<?php the_ID(); ?>" <?php post_class( $layout_type_class ); ?>>
	...
	<?php if ( is_singular( get_post_type() ) ) : // Is single view ?>
		...
		if ( get_theme_mod( 'post_feat_image', false ) == false ) { 
			if ( '' != $gallery ) :
				meteorite_the_gallery_slider( $gallery, '<div class="single-thumb">', '</div>' );
			...
			<?php if ( 'img-left' == $layout_type ) : // Specific HTML for img-left layout type ?>
				...
				<?php if ( ( is_home() || is_archive() ) && get_theme_mod( 'index_feat_image', false ) == false || is_search() && get_theme_mod( 'search_feat_image', false ) == false ) : ?>
					<div class="entry-thumb">
						<?php
						if ( '' != $gallery ) :
							meteorite_the_gallery_slider( $gallery );
						elseif ( has_post_thumbnail() ) :
							?>
