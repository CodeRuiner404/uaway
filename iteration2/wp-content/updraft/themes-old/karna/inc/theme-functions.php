...
if ( ! function_exists( 'karna_get_setting' ) ) {
	/**
	 * A wrapper function to get our settings.
	 *
	 */
	function karna_get_setting( $setting ) {
		$karna_settings = wp_parse_args(
			get_option( 'karna_settings', array() ),
			karna_get_defaults()
		);

		return $karna_settings[ $setting ];
	}
}

if ( ! function_exists( 'karna_get_layout' ) ) {
	/**
	...
	function karna_get_layout() {
		...
		$layout = $karna_settings['layout_setting'];

		// Get the individual page/post sidebar metabox value
		$layout_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_karna-sidebar-layout-meta', true ) : '';
		...
		if ( is_single() && ! $buddypress ) {
			$layout = null;
			$layout = $karna_settings['single_layout_setting'];
		}
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = null;
			$layout = $karna_settings['blog_layout_setting'];
		}

		// Finally, return the layout
		return apply_filters( 'karna_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'karna_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function karna_get_footer_widgets() {
		// Get current post
		global $post;
		...
		$widgets = $karna_settings['footer_widget_setting'];

		// Get the individual footer widget metabox value
		$widgets_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_karna-footer-widget-meta', true ) : '';
		...
		// Finally, return the layout
		return apply_filters( 'karna_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'karna_show_excerpt' ) ) {
	/**
	 * Figure out if we should show the blog excerpts or full posts
	 *
	 */
	function karna_show_excerpt() {
		// Get current post
		global $post;
		...
		$more_tag = apply_filters( 'karna_more_tag', strpos( $post->post_content, '<!--more-->' ) );

		// Check the post format
		$format = ( false !== get_post_format() ) ? get_post_format() : 'standard';

		// Get the excerpt setting from the Customizer
		$show_excerpt = ( 'excerpt' == $karna_settings['post_content'] ) ? true : false;
		...
		// Return our value
		return apply_filters( 'karna_show_excerpt', $show_excerpt );
	}
}

if ( ! function_exists( 'karna_show_title' ) ) {
	/**
	 ...
	 */
	function karna_show_title() {
		return apply_filters( 'karna_show_title', true );
	}
}

if ( ! function_exists( 'karna_padding_css' ) ) {
	/**
	 ...
	 */
	function karna_padding_css( $top, $right, $bottom, $left ) {
...
if ( ! function_exists( 'karna_get_link_url' ) ) {
	/**
	 * Return the post URL.
	 *
...
if ( ! function_exists( 'karna_get_navigation_location' ) ) {
	/**
	 ...
	 */
	function karna_get_navigation_location() {
		return apply_filters( 'karna_navigation_location', karna_get_setting( 'nav_position_setting' ) );
	}
}
