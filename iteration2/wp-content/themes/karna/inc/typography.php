...
if ( ! function_exists( 'karna_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'karna_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function karna_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', karna_typography_default_fonts() );
		...
		$font_settings = array(
			...
			'font_site_title',
			'font_site_tagline',
			'font_blog_header_title',
			'font_blog_header_text',
			'font_navigation',
			'font_widget_title',
			'font_buttons',
			'font_heading_1',
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $karna_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $karna_settings[$key], ':' ) !== false ) {
					$karna_settings[$key] = current( explode( ':', $karna_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $karna_settings[$key] );

				// Grab the variants using the plain name
				$variants = karna_get_google_font_variants( $karna_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'karna_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'karna_fonts_subset','' );
		...
		$default_fonts = KARNA_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('karna-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('karna-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'karna_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'karna_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function karna_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'karna_fonts_customize_register' ) ) {
			// Bail if Karna Premium is active
			return;
		}
		...
		$defaults = karna_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Karna_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'karna_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'karna_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'karna_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Karna_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'karna' ),
					'settings' => array(
						'family' => 'karna_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'karna_settings[body_font_weight]',
						'transform' => 'karna_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_control(
			new Karna_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'karna' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'karna_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'karna_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'karna_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Karna_Range_Slider_Control(
				...
				array(
					'type' => 'karna-range-slider',
					'description' => __( 'Line height', 'karna' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'karna_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'karna_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Karna_Range_Slider_Control(
				...
				array(
					'type' => 'karna-range-slider',
					'description' => __( 'Paragraph margin', 'karna' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'karna_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'karna_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function karna_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'karna_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'karna_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'karna_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_karna_get_all_google_fonts_ajax', 'karna_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function karna_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'karna_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'karna_customize_nonce' ] ), 'karna_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'karna_customize_nonce', 'karna_customize_nonce' );
		...
		$fonts = karna_get_all_google_fonts();
...
if ( ! function_exists( 'karna_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: karna_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function karna_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, karna_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'karna_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'karna_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: karna_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function karna_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'karna_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function karna_get_font_family_css( $font, $settings, $default ) {
		$karna_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'karna_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $karna_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'karna_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . karna_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'karna_add_to_font_customizer_list' ) ) {
	add_filter( 'karna_typography_customize_list', 'karna_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function karna_add_to_font_customizer_list( $fonts ) {
		// Bail if we don't have our defaults
		if ( ! function_exists( 'karna_get_default_fonts' ) ) {
			return;
		}
		...
		$all_fonts = karna_get_all_google_fonts();
		$select_fonts = karna_get_all_google_fonts( apply_filters( 'karna_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $karna_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $karna_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $karna_settings[ $setting ], karna_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $karna_settings[ $setting ] ) ) ] = array(
				'name' => $karna_settings[ $setting ],
		...
		if ( function_exists( 'karna_secondary_nav_get_defaults' ) ) {
			...
			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], karna_typography_default_fonts() ) ) {
