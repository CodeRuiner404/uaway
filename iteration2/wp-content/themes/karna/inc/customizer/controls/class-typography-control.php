...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Karna_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Karna_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'karna-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'karna-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), KARNA_VERSION, true );
			wp_enqueue_style( 'karna-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), KARNA_VERSION );

			wp_enqueue_script( 'karna-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'karna-typography-selectWoo' ), KARNA_VERSION, true );
			wp_enqueue_style( 'karna-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), KARNA_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'karna_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'karna' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'karna' );
			$this->json[ 'google_fonts' ] = apply_filters( 'karna_typography_customize_list', karna_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = karna_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'karna' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'karna' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'karna' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'karna' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="karna-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="karna-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="karna-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="karna-font-transform">
					<label>
