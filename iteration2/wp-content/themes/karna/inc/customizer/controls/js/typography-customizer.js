( function( api ) {
	...
	api.controlConstructor['karna-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.karna-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the karna_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'karna_get_all_google_fonts_ajax',
								karna_customize_nonce: karna_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.karna-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.karna-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.karna-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.karna-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.karna-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.karna-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.karna-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.karna-font-variant' );
			...
			control.container.on( 'change', '.karna-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.karna-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.karna-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.karna-font-family select' ).selectWoo();
	jQuery( '.karna-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".karna-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.karna-font-variant' ).hide();
		}
	});
